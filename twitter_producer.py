...
def generate_tweet():
    ...
    if sentiment == 'positive':
        text = random.choice([
            f"Loving the new product from {faker.company()}! Great job team! 👏",
            f"Had an amazing lunch at {faker.company()} — highly recommend! 😍",
        ...
        text = random.choice([
            f"Really disappointed with service at {faker.company()} today. 👎",
            f"Ugh... Monday meetings with {faker.name()} again 😤",
            f"Why does {faker.company()} always screw things up?",
            f"Terrible experience using {faker.catch_phrase()} 😞",
            f"{faker.name()} forgot the deadline. Not again. 😑"
        ])
    else:  # neutral
        text = random.choice([
            f"Just arrived at {faker.city()} for a conference. 🧳",
            f"Reading up on {faker.bs()} today.",
            f"{faker.name()} mentioned an interesting idea at lunch.",
